<?php
if (!function_exists('ttk_promotion')) {
function ttk_promotion($theme='',$array='',$file='') {
global ${$theme};
if ($theme == '' or $array == '' or $file == '') {
die ('No theme name, theme option, or parent defined in Theme Toolkit');
}
${$theme} = new ttk_promotion($theme,$array,$file);
}
}
?>
...
add_submenu_page('ttk_functions.php', __('Stallion SEO Theme Promotion Options','stallion-menu'), __('Promotion Options','stallion-menu'), 'manage_options', 'ttk_promotion_functions.php', array(&$this,'admin_menu'));
}
function do_init() {
$themes = get_themes();
...
$options = get_option('theme-'.$this->infos['theme_shortname'].'-promotion');
...
update_option('theme-'.$this->infos['theme_shortname'].'-promotion','');
if (update_option('theme-'.$this->infos['theme_shortname'].'-promotion',$array)) {
...
echo '<meta http-equiv="refresh" content="0;URL=admin.php?page=ttk_promotion_functions.php">';
echo "<script> self.location(\"admin.php?page=ttk_promotion_functions.php\");</script>";
if ($this->infos['active']) {
...
global $wpdb;
$where = 'theme-'.$this->infos['theme_shortname'].'-promotion';
...
add_option('theme-'.$this->infos['theme_shortname'].'-promotion',$options, 'Options for theme '.$this->infos['theme_name']);
...
echo "<style type='text/css'>input[type=text] { width: 200px;}</style><h2>Configure Stallion SEO Theme : Promotion Options</h2>";
