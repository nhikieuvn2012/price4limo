<?php
if (!function_exists('ttk_clickbank')) {
function ttk_clickbank($theme='',$array='',$file='') {
global ${$theme};
if ($theme == '' or $array == '' or $file == '') {
die ('No theme name, theme option, or parent defined in Theme Toolkit');
}
${$theme} = new ttk_clickbank($theme,$array,$file);
}
}
?>
...
add_submenu_page('ttk_functions.php', __('Stallion SEO Theme Clickbank Options','stallion-menu'), __('Clickbank Options','stallion-menu'), 'manage_options', 'ttk_clickbank_functions.php', array(&$this,'admin_menu'));
}
function do_init() {
$themes = get_themes();
...
$options = get_option('theme-'.$this->infos['theme_shortname'].'-clickbank');
...
update_option('theme-'.$this->infos['theme_shortname'].'-clickbank','');
if (update_option('theme-'.$this->infos['theme_shortname'].'-clickbank',$array)) {
...
echo '<meta http-equiv="refresh" content="0;URL=admin.php?page=ttk_clickbank_functions.php">';
echo "<script> self.location(\"admin.php?page=ttk_clickbank_functions.php\");</script>";
if ($this->infos['active']) {
...
global $wpdb;
$where = 'theme-'.$this->infos['theme_shortname'].'-clickbank';
...
add_option('theme-'.$this->infos['theme_shortname'].'-clickbank',$options, 'Options for theme '.$this->infos['theme_name']);
...
echo "<style type='text/css'>input[type=text] { width: 200px;}</style><h2>Configure Stallion SEO Theme : Clickbank Options</h2>";
