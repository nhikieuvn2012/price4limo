<?php
if (!function_exists('ttk_adsense')) {
function ttk_adsense($theme='',$array='',$file='') {
global ${$theme};
if ($theme == '' or $array == '' or $file == '') {
die ('No theme name, theme option, or parent defined in Theme Toolkit');
}
${$theme} = new ttk_adsense($theme,$array,$file);
}
}
?>
...
add_submenu_page('ttk_functions.php', __('Stallion SEO Theme AdSense Options','stallion-menu'), __('AdSense Options','stallion-menu'), 'manage_options', 'ttk_adsense_functions.php', array(&$this,'admin_menu'));
}
function do_init() {
$themes = get_themes();
...
$options = get_option('theme-'.$this->infos['theme_shortname'].'-adsense');
...
update_option('theme-'.$this->infos['theme_shortname'].'-adsense','');
if (update_option('theme-'.$this->infos['theme_shortname'].'-adsense',$array)) {
...
echo '<meta http-equiv="refresh" content="0;URL=admin.php?page=ttk_adsense_functions.php">';
echo "<script> self.location(\"admin.php?page=ttk_adsense_functions.php\");</script>";
if ($this->infos['active']) {
...
global $wpdb;
$where = 'theme-'.$this->infos['theme_shortname'].'-adsense';
...
add_option('theme-'.$this->infos['theme_shortname'].'-adsense',$options, 'Options for theme '.$this->infos['theme_name']);
...
echo "<style type='text/css'>input[type=text] { width: 200px;}</style><h2>Configure Stallion SEO Theme : AdSense Options</h2>";
