<?php
if (!function_exists('ttk_layout')) {
function ttk_layout($theme='',$array='',$file='') {
global ${$theme};
if ($theme == '' or $array == '' or $file == '') {
die ('No theme name, theme option, or parent defined in Theme Toolkit');
}
${$theme} = new ttk_layout($theme,$array,$file);
}
}
?>
...
add_submenu_page('ttk_functions.php', __('Stallion SEO Theme Layout Options','stallion-menu'), __('Layout Options','stallion-menu'), 'manage_options', 'ttk_layout_functions.php', array(&$this,'admin_menu'));
}
function do_init() {
$themes = get_themes();
...
$options = get_option('theme-'.$this->infos['theme_shortname'].'-layout');
...
update_option('theme-'.$this->infos['theme_shortname'].'-layout','');
if (update_option('theme-'.$this->infos['theme_shortname'].'-layout',$array)) {
...
echo '<meta http-equiv="refresh" content="0;URL=admin.php?page=ttk_layout_functions.php">';
echo "<script> self.location(\"admin.php?page=ttk_layout_functions.php\");</script>";
if ($this->infos['active']) {
...
global $wpdb;
$where = 'theme-'.$this->infos['theme_shortname'].'-layout';
...
add_option('theme-'.$this->infos['theme_shortname'].'-layout',$options, 'Options for theme '.$this->infos['theme_name']);
...
echo "<style type='text/css'>input[type=text] { width: 200px;}</style><h2>Configure Stallion SEO Theme : Layout Options</h2>";
