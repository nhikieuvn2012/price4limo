<?php
if (!function_exists('ttk_colours')) {
function ttk_colours($theme='',$array='',$file='') {
global ${$theme};
if ($theme == '' or $array == '' or $file == '') {
die ('No theme name, theme option, or parent defined in Theme Toolkit');
}
${$theme} = new ttk_colours($theme,$array,$file);
}
}
?>
...
add_submenu_page('ttk_functions.php', __('Stallion SEO Theme Colour Options','stallion-menu'), __('Colour Options','stallion-menu'), 'manage_options', 'ttk_colours_functions.php', array(&$this,'admin_menu'));
}
function do_init() {
$themes = get_themes();
...
$options = get_option('theme-'.$this->infos['theme_shortname'].'-colours');
...
update_option('theme-'.$this->infos['theme_shortname'].'-colours','');
if (update_option('theme-'.$this->infos['theme_shortname'].'-colours',$array)) {
...
echo '<meta http-equiv="refresh" content="0;URL=admin.php?page=ttk_colours_functions.php">';
echo "<script> self.location(\"admin.php?page=ttk_colours_functions.php\");</script>";
if ($this->infos['active']) {
...
global $wpdb;
$where = 'theme-'.$this->infos['theme_shortname'].'-colours';
...
add_option('theme-'.$this->infos['theme_shortname'].'-colours',$options, 'Options for theme '.$this->infos['theme_name']);
...
echo "<style type='text/css'>input[type=text] { width: 200px;}</style><h2>Configure Stallion SEO Theme : Colours Options</h2>";
