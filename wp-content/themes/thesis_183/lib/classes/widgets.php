...
?>
	<form method="get" class="search_form" action="<?php bloginfo('url'); ?>/">
		<p>
			<input class="text_input" type="text" value="<?php esc_html_e($field_value); ?>" name="s" id="s" onfocus="if (this.value == '<?php esc_html_e($field_value); ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php esc_html_e($field_value); ?>';}" />
...
class thesis_search_widget extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		if ($title)
			printf('%s', $before_title . esc_attr($title) . $after_title);
?>
	<form method="get" class="search_form" action="<?php printf('%s', esc_url(bloginfo('url'))); ?>">
		<p>
			<input class="text_input" type="text" value="<?php esc_html_e($default_value); ?>" name="s" id="s" onfocus="if (this.value == '<?php echo esc_html($default_value); ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php echo esc_html($default_value); ?>';}" />
...
class thesis_widget_subscriptions extends WP_Widget {
	...
	function widget($args, $instance) {
		extract($args);
		$description = !empty($instance['description']) ? "<p>" . wp_kses_data($instance['description']) . "</p>\n" : '';
	...
	function widget($args, $instance) {
		...
		while ($thesis_kre_query->have_posts()) {
			...
			$out .= "<li><a href=\"" . esc_url(get_permalink($thesis_kre_query->post->ID)) . "\" title=\"" . __('Click to read ', 'thesis')
			 	 . esc_attr__($thesis_kre_query->post->post_title, 'thesis') . "\" rel=\"bookmark\">" . esc_attr__($thesis_kre_query->post->post_title, 'thesis') . "</a>";
			if ($comms == 1) {
...
function thesis_update_all_widgets() {
	...
	if (! empty($alloptions['theme_mods_thesis_183'])) {
