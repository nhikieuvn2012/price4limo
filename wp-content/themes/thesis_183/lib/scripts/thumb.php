...
******* ATTENTION, WP MULTISITE/NETWORK USERS ********

--> We are assuing an upload path of wp-content/blogs.dir/{blog-id}/files and that a particular site will be set up so that file URLs are site.com/files/whatever/item.ext
----> If this isn't the schema you're using, please contact Matt G at mattonomics@gmail.com.
------> Please, do not contact the support forum staff as they will not be able to help with this issue.

*/

if (! empty($_GET['multisite']))
	define('DOING_MULTISITE', true);
...
$cache_dir = thesis_multisite();
/* multisite support */
function thesis_multisite($c = '../../custom/cache') {
	if (defined('DOING_MULTISITE') && DOING_MULTISITE) {
		preg_match_all('/[0-9]+/', $_GET['multisite'], $result); // sanity check
		$hopeful = sprintf('%d', implode('', $result[0])); // further sanity
		if (file_exists('../../custom-' . $hopeful . '/cache')) // if it's not there, fall back on the default
			$c = '../../custom-' . $hopeful . '/cache';
	}
	return $c;
}
...
function cleanSource($src) {
	...
	if (defined('DOING_MULTISITE') && DOING_MULTISITE) {
		$src = stristr($src, '/files');
		$blog_id = abs(intval($_GET['multisite']));
		$src = "wp-content/blogs.dir/$blog_id$src";
	}
