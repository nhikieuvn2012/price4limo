...
function thesis_archive_intro($depth = 3) {
	...
	if ($wp_query->is_category || $wp_query->is_tax || $wp_query->is_tag) { #wp
		$headline = ! empty($thesis_terms->terms) ? trim(wptexturize(($thesis_terms->terms[$wp_query->queried_object->taxonomy][$wp_query->queried_object->term_id]['headline']) ? stripslashes($thesis_terms->terms[$wp_query->queried_object->taxonomy][$wp_query->queried_object->term_id]['headline']) : $wp_query->queried_object->name)) : false; #wp
